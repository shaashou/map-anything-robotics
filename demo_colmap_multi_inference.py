...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional numpy file (.npy) containing camera intrinsics. Can be a single (3,3) matrix or an array of shape (N,3,3)",
    )
    parser.add_argument(
        "--intrinsics_dir",
        type=str,
        default=None,
        help="Optional directory containing per-image intrinsics saved as .npy files named like the image basename (e.g. img01.jpg -> img01.npy)",
    )
    parser.add_argument(
        "--depth_dir",
        type=str,
        default=None,
        help="Optional directory containing per-image depth maps saved as .npy files named like the image basename (H x W float32)",
    )
    parser.add_argument(
        "--conf_thres_value",
        type=float,
...
def run_mapanything(
    ...
    if views is not None:
        predictions = model.infer(
            ...
            apply_mask=True,
            mask_edges=True,
            # 如果是True的话 代表着过滤置信度低的点 可以尝试设置为True
...
def demo_fn(args):
    ...
    mapanything_fixed_resolution = (518,294)
    ...
    img = cv2.imread('/home/zyk/projection/Dr-Splat/example/images/0.png')
    intrinsics = np.array([
        [920.7479858398438, 0.0, 649.4222412109375],
        [0.0, 921.3973999023438, 349.3091735839844],
        [0.0, 0.0, 1.0]
    ], dtype=np.float32)
    depth_z = np.load('/home/zyk/projection/Dr-Splat/example/depth/0.npy').astype(np.float32) #720 1280
    views = [{
    "img": img,  # [1, 3, H, W]
    "depth_z": depth_z,  # [1, H, W] 或 [1, 1, H, W]
    "intrinsics": intrinsics,  # [1, 3, 3]
    "data_norm_type": ["dinov2"],
    }]
    views = preprocess_inputs(views) 
    # Run MapAnything to estimate camera and depth
    # Run with 518 x 294 images
    ...
    if args.use_ba:
        ...
        image_size = np.array(mapanything_fixed_resolution)
        print(image_size)
        num_frames, height, width, _ = points_3d.shape
        # print(height, width)
        # Denormalize images before computing RGB values 1024 1024 518 294
        points_rgb_images = F.interpolate(
            images,
            size=mapanything_fixed_resolution,
            mode="bilinear",
            align_corners=False,
        )
        ...
        points_rgb_list = []
        for i in range(img_no_norm.shape[0]):
            rgb_img = img_no_norm[i]
            # 如果是 float32 且最大值 <= 1.0，说明是 0~1，需要乘 255
        ...
        points_rgb = np.stack(points_rgb_list)  # (N, H, W, 3)
        ...
        points_3d = points_3d.reshape(-1, 3)[conf_mask_flat]
        points_xyf = points_xyf.reshape(-1, 3)[conf_mask_flat]
        points_rgb = points_rgb.reshape(-1, 3)[conf_mask_flat]
        point_conf = depth_conf.flatten()[conf_mask_flat]  # 新增置信度
        ...
        if reconstruction is None:
            raise ValueError("COLMAP reconstruction failed, please check your inputs.")
    ...
    # 保存点云置信度
    np.save(
        os.path.join(args.scene_dir, "sparse/points_with_conf.npy"),
        {
            "points_3d": points_3d,
            "points_rgb": points_rgb,
            "points_conf": point_conf
        }
    )
...
def rename_colmap_recons_and_rescale_camera(
    ...
    for pyimageid in reconstruction.images:
        ...
        if rescale_camera:
            ...
            resize_ratio = max(real_image_size) / max(img_size)
